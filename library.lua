	...
	function sec:AddSection(Texto,side)
		...
		function inside:AddToggle(Text,Enabled,keybind,Callback)
			...
			if keybind then
				...
				local keybindMode = "Toggle" -- Default mode: Toggle, Hold, Always

				-- Create right-click menu
				local KeyMenu = Instance.new("Frame")
				KeyMenu.Name = "KeyMenu"
				KeyMenu.Parent = PCR_1
				KeyMenu.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
				KeyMenu.BorderSizePixel = 0
				KeyMenu.Size = UDim2.new(0, 100, 0, 90)
				KeyMenu.Visible = false
				KeyMenu.ZIndex = 100

				local KeyMenuCorner = Instance.new("UICorner")
				KeyMenuCorner.CornerRadius = UDim.new(0, 4)
				KeyMenuCorner.Parent = KeyMenu

				local KeyMenuStroke = Instance.new("UIStroke")
				KeyMenuStroke.Color = Color3.fromRGB(91, 133, 197)
				KeyMenuStroke.Thickness = 1
				KeyMenuStroke.Parent = KeyMenu

				local function CreateMenuOption(text, mode)
					local option = Instance.new("TextButton")
					option.Name = text
					option.Parent = KeyMenu
					...
					option.MouseEnter:Connect(function()
						option.BackgroundColor3 = Color3.fromRGB(84, 122, 181)
				...
				CreateMenuOption("Toggle", "Toggle")
				CreateMenuOption("Hold", "Hold")
				CreateMenuOption("Always", "Always")

				local UIListLayout = Instance.new("UIListLayout")
				UIListLayout.Parent = KeyMenu
				UIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
				...
				KeyButton.MouseButton2Click:Connect(function()
					if not ischanging then
						...
						KeyMenu.Position = UDim2.new(0, mousePos.X, 0, mousePos.Y)
					...
					if not gp then
						if (a.KeyCode.Name == KeyCode or a.KeyCode.Name == KeyCode.Name) and ischanging == false then
							...
							elseif keybindMode == "Hold" then
								SetState(true)
							elseif keybindMode == "Always" then
								-- Always mode - toggle on key press
								pcall(function()
									Update()
								end)
							end
